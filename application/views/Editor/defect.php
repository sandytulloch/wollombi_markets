...
<!-- <div class='container'> -->
	<h4><a href="<?= base_url();?>Editor/structure/<?=$structure['ID']?>"><?=$structure['Name']?></a> - > <a href="<?= base_url();?>Editor/group/<?=$group['ID']?>"><?=$group['Type']?> <?=$group['Index']?></a> - > <a href="<?= base_url();?>Editor/component/<?=$component['ID']?>"><?=$component['Type']?> <?=$component['Index']?></a> - > <?=$defect['Type']?></h4>
	<form class="form-horizontal" role="form" action='<?= base_url();?>Editor/save_defect' method="post">
		<? $i = 0 ?>

		<? $columns = 3; ?>

		<?php foreach ($defect as $key=>$value): ?>
			<?php if($i % $columns == 0): ?>
				<div class='row'>
			<?php endif; ?>
	...
	<div class='row'>
		<div id="myCarousel" class="carousel slide col-xs-6" data-ride="carousel">
		  <!-- Indicators -->
